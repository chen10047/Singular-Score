###########################################
# Based on NAS-Bench-201    #
# Original work: Dong & Yang, ICLR 2020  #
##############################################################################
...
"""The API for size search space(sss) in NATS-Bench."""
import collections
import copy
import os
import random
...
class NATSsize(NASBenchMetaAPI):
    ...
    def __init__(
        ...
        self._all_base_names = ALL_BASE_NAMES # ['cifar10','cifar100']
        self.filename = None
        ...
        if isinstance(file_path_or_dict, dict):
            ...
            self.arch2infos_dict = collections.OrderedDict() # 有序字典 存储架构索引 -> 超参数 -> 评估架构映射关系
            self._avaliable_hps = set()
    ...
    def get_more_info(
        ...
        index = self.query_index_by_arch(
            index
        )  # 标准化 To avoid the input is a string or an instance of a arch object
        ...
        if dataset == "cifar10-valid":
            ...
            except Exception as unused_e:  # None pylint: disable=broad-except
                test_info = None
            try:
                valid_info = archresult.get_metrics(
                    dataset, "x-valid", iepoch=iepoch, is_random=is_random
